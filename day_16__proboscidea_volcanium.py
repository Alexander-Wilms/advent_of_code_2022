        ...
        if time < max_time:
            ...
            for pot_next_valve_name in possible_tunnels:
                for potential_open_valve in [0, 1]:
                    if (
                        potential_open_valve
                        and pot_next_valve_name not in open_valves.keys()
                        and pot_next_valve_name == current_valve
                        and valves[pot_next_valve_name].flow_rate > 0
                    ) or (not potential_open_valve):
                        ...
                        qprint(
                            indent
                            + f"open valves so far: {pot_open_valves[pot_next_valve_name][potential_open_valve]}"
                        )
                        ...
                        if total_pressure >= max_pressure:
                            ...
                            possible_max_open_valves = deepcopy(
                                pot_open_valves[pot_next_valve_name][
                                    potential_open_valve
                                ]
                            )
                            possible_path_next_path = deepcopy(
                                pot_paths[pot_next_valve_name][potential_open_valve]
                            )
...
if True:
    path_input = []
    path_result, open_valves = find_optimal_path(
        0, 4, first_valve, path_input, 0, 0, dict()
    )
    pprint(open_valves)
    pprint(path_result)
...
if True:
    path_input = []
    path_result, open_valves = find_optimal_path(
        0, 6, first_valve, path_input, 0, 0, dict()
    )
    if True:
        pprint(open_valves)
        pprint(path_result)
