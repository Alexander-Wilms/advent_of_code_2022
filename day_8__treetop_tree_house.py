...
def is_visible_from_direction(tree_map: numpy.ndarray, row, col, direction: str) -> bool:
    print('is_visible_from_direction()')
    height_of_tree_to_check = tree_map[row, col]

    (rows, cols) = tree_map.shape
    match direction:
        case '→':
            tree_map_aligned = tree_map
            aligned_row = row
            aligned_col = col
        case '↑':
            tree_map_aligned = numpy.rot90(tree_map, axes=(1, 0))
            aligned_row = col
            aligned_col = rows-row-1
        case '←':
            tree_map_aligned = numpy.rot90(tree_map, k=2, axes=(1, 0))
            aligned_row = rows-row-1
            aligned_col = cols-col-1
        case '↓':
            tree_map_aligned = numpy.rot90(tree_map, k=3, axes=(1, 0))
            aligned_row = cols-col-1
            aligned_col = row

    print(direction)
    #print_map(tree_map_aligned, True, aligned_row, aligned_col)

    visible = True

    print(height_of_tree_to_check)

    for idx in range(aligned_col):
        #print('checking col '+str(idx))
